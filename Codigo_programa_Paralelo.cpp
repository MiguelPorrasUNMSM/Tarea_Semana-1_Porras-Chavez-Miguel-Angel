...
int main() {
    // Generar el mazo de 4 millones de cartas
    auto mazo = generarMazo(4000000);

    // Barajar el mazo
    shuffle(mazo.begin(), mazo.end(), mt19937{random_device{}()});

    // Medir el tiempo de ordenación paralela
    auto inicio = chrono::high_resolution_clock::now();
    sort(execution::par, mazo.begin(), mazo.end(), compararCartas);
    auto fin = chrono::high_resolution_clock::now();

    // Mostrar el tiempo tomado para la ordenación
    cout << "Tiempo para ordenación paralela: "
